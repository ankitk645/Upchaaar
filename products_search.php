...
?>
<?php 
$products_search=$_REQUEST['Product'];

if($products_search!=TRUE)
{
...
<?php
$con=new mysqli('localhost','upchaaar_root','babloo@098765','upchaaar_meds');
$query= $con->query("select * from products where name like '%$products_search%'");
$row = $query->fetch_assoc();                                     
?>
<?php
/*
$query= mysql_query("select * from products where name like '%$products_search%' ");
$row=mysql_fetch_array($query);
if($row['name']!=TRUE)
{
	$no_result="No Results Found! wanna try again?";
}
*/
?>
<!DOCTYPE html>
<html>
<head>
<title>"<?php echo $products_search;?>" Upchaaar.com: Indian Online Pharmacy | Buy Medicines Online, Fast Delivery</title>
<!-- for-mobile-apps -->
...
<!-- banner -->
	<div class="banner">
		...
		<div class="w3l_banner_nav_right">
			...
			<?php
                                        $con=new mysqli('localhost','upchaaar_root','babloo@098765','upchaaar_meds');
                                        $query= $con->query("select * from products where name like '%$products_search%' ORDER by id DESC LIMIT 20 ");
								...
								<figure>
									...
									<div class="snipcart-item block" >
										<div class="snipcart-thumb">
											...
											<a href="products_info.php?p_id=<?php echo $row3['id'];?>"><img title=" " alt=" " src="<?php echo $row3['image'];?>" /></a>		
											<p><?php echo $row3['name'];?></p>
											<h4>Rs.<?php echo $row3['offer_price'];?> <span>Rs.<?php echo $row3['normal_price'];?></span></h4>
										</div>
										<div class="snipcart-details top_brand_home_details">
											<form action="#" method="post">
												<fieldset>
													...
													<input type="hidden" name="amount" value="<?php echo $row3['normal_price'];?>" />
